...
class ProdukController {
    ...
    // Edit User
    public function createProduk($judulHome,$deskripsiJudul,$gambar,$status) {
        $response = [
            "success" => true,
            "message" => [],
        ];
        ...
        $query = "INSERT INTO produk (judul_home, deskripsi_judul, gambar, status) VALUES (?,?,'',?)";
        ...
        $stmt->bind_param("sss", $judulHome, $deskripsiJudul, $status);
        $stmt->execute();
    ...
    public function editProduk($idHome,$judulHome,$deskripsiJudul,$gambar,$gambarDatabase,$status) {
        ...
        if (!$uploadResult['success']) {
            ...
            $response["message"][] = $uploadResult["message"];
            $gambar = $gambarDatabase;  // Use the default or previously set photo path
        } else {
            // If file upload is successful, update the file path
            $gambar = $uploadResult["filePath"];
            $update = $this -> connection -> prepare('UPDATE produk SET gambar = ? WHERE id_home = ?');
    ...
    private function handleFileUpload($gambar, $new_id) {
        ...
        $uploadDirectory = "img/produk/";
        $ext = pathinfo($gambar["name"], PATHINFO_EXTENSION);
        $newFileName = 'produk' . $new_id . '.' . $ext;
